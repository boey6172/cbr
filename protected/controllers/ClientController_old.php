		...
		{		
			...
			if($reservation->reservation_type == 1 || $reservation->reservation_type == 2)
			{
				...
				$findCars = Car::model()->findAll(array(
					'condition'=>'passenger_capacity >= :passenger_capacity  AND car_id IN (' . $filtered_cars . '0)',
			...
			else
			{
				...
				$findCars = Car::model()->findAll(array(
					'condition'=>'passenger_capacity >= :passenger_capacity  AND car_id IN (' . $filtered_cars . '0) AND car_id NOT IN (' . $filtered_cars_in_reservation . '0)',
					'params'=>array(':passenger_capacity'=>$reservation->no_of_passengers)
					//AND distance_capacity = :distance_capacity
					//,':distance_capacity'=>$reservation->distance removed temporarrily
				));
